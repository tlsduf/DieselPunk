...
void FAnimNode_DragonWarpSolver::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	if (enable_solver && Output.Pose.GetPose().IsValid() && (FVector(0, 0, 0) - Output.AnimInstanceProxy->GetActorTransform().GetScale3D()).Size() > 0 && FAnimWeight::IsRelevant(ActualAlpha) && IsValidToEvaluate(Output.AnimInstanceProxy->GetSkeleton(), Output.AnimInstanceProxy->GetRequiredBones()) && dragon_limb_input.Num() > 0 && Output.ContainsNaN() == false)
	{
		...
		if(Hip_Bone_Ref.IsValidToEvaluate())
		{
